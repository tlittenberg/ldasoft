/*   Compile:
 gcc Full_Residual.c Subroutines.c arrays.c -lm -lgsl -o Full_Residual
 // :tlittenb:20110110
 */
...
int main(int argc, char *argv[])
{
  ...
  if(argc!=4)
  {
    printf("./FullResidual Catalogue Orbits TOBS\n");
    return 0;
  }
  ...
  printf(" Source file contains %i sources\n",NS);
  ...
  for(n=0; n<NS; n++)
  {
    ...
    params[n][6] = phase;
    params[n][7] = fdot;
    params[n][8] = 11.0/3.0*fdot*fdot/f;
  }
  fclose(infile);
  ...
  for(i=0; i<NFFT;  i++) h[0][i] = h[1][i] = 0.0;
