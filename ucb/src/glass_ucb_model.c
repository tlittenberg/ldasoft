...
{
    struct Model *model_temp = malloc(sizeof(struct Model));
    alloc_model(data, model_temp, model->Nmax);
    copy_model(model, model_temp);

    for(int n=0; n<data->N; n++)
    {
        model_temp->tdi->X[n]=0.0;
        model_temp->tdi->Y[n]=0.0;
        model_temp->tdi->Z[n]=0.0;
    }
    
    //Loop over signals in model
    for(int n=0; n<model_temp->Nlive; n++)
    {
        struct Source *source_temp = model_temp->source[n];
        
        for(int i=0; i<data->N; i++)
        {
            source_temp->tdi->X[i]=0.0;
            source_temp->tdi->Y[i]=0.0;
            source_temp->tdi->Z[i]=0.0;
            source_temp->list[i]=0;
        }
    ...
    double logL = gaussian_log_likelhood_wavelet(data, model);
    double logL_temp = gaussian_log_likelhood_wavelet(data, model_temp);
    double match = waveform_match_wavelet(model->source[0], model_temp->source[0], model->noise);
