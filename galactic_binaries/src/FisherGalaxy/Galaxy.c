  ...
  rseed = -924514346;
    ...
    while ( !feof(Infile) )
       ...
       if((fdot > 0.0) || (alpha < 0.1))  // keep all detached and 10% of AMCVn
	 {
       ...
       if(noiseFlag==1)instrument_noise(f, LISAorbit->fstar, LISAorbit->L, &SAE, &SXYZ);
		 if(noiseFlag==2)instrument_noise2(f, LISAorbit->fstar, LISAorbit->L, &SAE, &SXYZ);
