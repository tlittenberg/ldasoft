...
f = h5py.File('/Users/klackeos/data/LDC1-4_GB_v1.hdf5', 'r')
#amp_ldc=[]
...
#f = h5py.File('/Users/klackeos/data/LDC1-3_GB_v1.hdf5', 'r')
...
f2=open('/Users/klackeos/ldasoft/master/bin/ldc_galaxy/first_run/6mo/chains/dimension_chain.dat.4','r')
...
sns.distplot(f0,bins=1000, kde=False,hist_kws={"histtype": "step", "linewidth": 3,"alpha": 1, "color": "g"})
plt.show()
...
freq_ldc=np.sort(freq_ldc[:])
#num_bins = 1000
#n, bins, patches = plt.hist(freq_ldc, num_bins, normed=1, facecolor='blue', alpha=0.5)
#freq_ldc[len(freq_ldc[freq_ldc<0.0099718]):len(freq_ldc[freq_ldc<0.00997725])]
#0.00997185, 0.00997438, 0.00997722
#srcs=freq_ldc[len(freq_ldc[freq_ldc<0.0099718]):len(freq_ldc[freq_ldc<0.00997725])]
srcs=freq_ldc[len(freq_ldc[freq_ldc<min(f0)/T]):len(freq_ldc[freq_ldc<max(f0)/T])]
freq_ldc=f['H5LISA']['GWSources']['GalBinaries']['Frequency']

gbmcmc_sources=[]
gbmcmc_sources.append(find_nearest(freq_ldc,srcs[0]))

gbmcmc_sources.append(find_nearest(freq_ldc,srcs[1]))

gbmcmc_sources.append(find_nearest(freq_ldc,srcs[2]))



#f0_hist=plt.hist(f0,bins=10)
#xx=f0_hist[1][0:len(f0_hist[0])]
#peak_freqs=xx[f0_hist[0]>0.0]

#or#
#peak_freqs=np.unique(np.round(f0/T,6))
...
#    f0_sources.append(g)
#    dfdt_sources.append(g)
#    amp_sources.append(g)
#    phi_sources.append(g)
#    costheta_sources.append(g)
#    cosi_sources.append(g)
#    psi_sources.append(g)
#    phi0_sources.append(g)
...
while r <= len(srcs):
    ...
    for k in range(0,len(f0),1):
        xx=find_nearest(srcs*T,f0[k])
#        print(xx,srcs*T,f0[k])
        if xx == r-1:
            f0_1.append(f0[k])
            dfdt_1.append(dfdt[k])
            amp_1.append(amp[k])
            phi_1.append(phi[k])
            costheta_1.append(costheta[k])
            cosi_1.append(cosi[k])
            psi_1.append(psi[k])
            phi0_1.append(phi0[k])
    #    elif xx == 1:
    #        f0_2.append(f0[k])
    #        dfdt_2.append(dfdt[k])
    #        amp_2.append(amp[k])
    #        phi_2.append(phi[k])
    #        costheta_2.append(costheta[k])
    #        cosi_2.append(cosi[k])
    #        psi_2.append(psi[k])
    #        phi0_2.append(phi0[k])
    #    else:
    #        f0_3.append(f0[k])
    #        dfdt_3.append(dfdt[k])
    #        amp_3.append(amp[k])
    #        phi_3.append(phi[k])
    #        costheta_3.append(costheta[k])
    #        cosi_3.append(cosi[k])
    #        psi_3.append(psi[k])
    #        phi0_3.append(phi0[k])



    f0=np.asarray(f0_1)
    dfdt=np.asarray(dfdt_1)
    amp=np.asarray(amp_1)
    phi=np.asarray(phi_1)
    costheta=np.asarray(costheta_1)
    cosi=np.asarray(cosi_1)
    psi=np.asarray(psi_1)
    phi0=np.asarray(phi0_1)


    #f0_sort=np.sort(f0)
    #f1=f0_sort[find_nearest(f0_sort,max(f0_sort))]
    #x=find_nearest(f0_sort,max(f0_sort))
    #f0_temp=f0_sort[0:x-9000]
    #f2=f0_temp[find_nearest(f0_temp,max(f0_temp))]
    #x=find_nearest(f0_temp,max(f0_temp))
    #f0_temp=f0_temp[0:x-26000]
    #f3=f0_temp[find_nearest(f0_temp,max(f0_temp))]
    #maxf=f0_sort[0]
    #for k in range(0,len(f0_sort),1):
    #    if f0_sort[k] > maxf:
    #(f0_sort,max(f0_sort))-10:find_nearest(f0_sort,max(f0_sort))+10]=0
    #f2=f0_temp[find_nearest(f0_temp,max(f0_temp))]
    #f0_temp[find_nearest(f0_temp,max(f0_temp))-5:find_nearest(f0_temp,max(f0_temp))+5]=0
    #f3=f0_temp[find_nearest(f0_temp,max(f0_temp))]
    ...
    if len(phi0_a) == len(phi0) or len(phi0_b) == len(phi0):
            ...
            target=open(os.path.join('/Users/klackeos/Desktop/','covariance_matrix_src'+str(r)+'.txt'),'w')
...
#    f0_1=np.asarray(f0_1)
#    dfdt_1=np.asarray(dfdt_1)
#    amp_1=np.asarray(amp_1)
#    phi_1=np.asarray(phi_1)
#    costheta_1=np.asarray(costheta_1)
#    cosi_1=np.asarray(cosi_1)
#    psi_1=np.asarray(psi_1)
#    phi0_1=np.asarray(phi0_1)
#
#    f0_2=np.asarray(f0_2)
#    dfdt_2=np.asarray(dfdt_2)
#    amp_2=np.asarray(amp_2)
#    phi_2=np.asarray(phi_2)
#    costheta_2=np.asarray(costheta_2)
#    cosi_2=np.asarray(cosi_2)
#    psi_2=np.asarray(psi_2)
#    phi0_2=np.asarray(phi0_2)
#
#
#    f0_3=np.asarray(f0_3)
#    dfdt_3=np.asarray(dfdt_3)
#    amp_3=np.asarray(amp_3)
#    phi_3=np.asarray(phi_3)
#    costheta_3=np.asarray(costheta_3)
#    cosi_3=np.asarray(cosi_3)
#    psi_3=np.asarray(psi_3)
#    phi0_3=np.asarray(phi0_3)
#
#
#
#
#    data1=np.vstack([f0_1/T,dfdt_1/T/T,exp(amp_1),phi_1,costheta_1,cosi_1,psi_1,phi0_1])
#    datanew1=np.transpose(data1)
#
#    data2=np.vstack([f0_2/T,dfdt_2/T/T,exp(amp_2),phi_2,costheta_2,cosi_2,psi_2,phi0_2])
#    datanew2=np.transpose(data2)
#
#    data3=np.vstack([f0_3/T,dfdt_3/T/T,exp(amp_3),phi_3,costheta_3,cosi_3,psi_3,phi0_3])
#    datanew3=np.transpose(data3)
#
#
#
#    rng=[(min(f0)/T,max(f0)/T), (min(dfdt)/T/T,max(dfdt)/T/T), (min(exp(amp)),max(exp(amp))), (min(phi),max(phi)), (min(costheta),max(costheta)), (min(cosi),max(cosi)), (min(psi),max(psi)), (min(phi0),max(phi0))]
#
#
#    c = ChainConsumer()
#    c.add_chain(datanew1, parameters=[r"$f0 (Hz)$", "df/dt", r"$amp$", r"$\phi$",r"$cos\theta$",r"$cos\iota$", r"$\psi$", r"$\phi_0$"],name="f1").configure(contour_labels="confidence")
#    c.add_chain(datanew2, parameters=[r"$f0 (Hz)$", "df/dt", r"$amp$", r"$\phi$",r"$cos\theta$",r"$cos\iota$", r"$\psi$", r"$\phi_0$"],name="f2")
#    c.add_chain(datanew3, parameters=[r"$f0 (Hz)$", "df/dt", r"$amp$", r"$\phi$",r"$cos\theta$",r"$cos\iota$", r"$\psi$", r"$\phi_0$"],name="f3")
#
#    c.configure(shade_gradient=[0.8,0.5,1.0],bins=25,colors=["#006b3c","#9cc6d6","#7c709c"], shade_alpha=0.8, linewidths=[1.0,1.0, 1.0], tick_font_size=8, max_ticks=4,shade=[True, True, True],summary=[True,True,True],kde=[False,False, False])
#    #c.configure(shade_gradient=[0.8],bins=25,colors=["c"], shade_alpha=0.8, linewidths=[1.0], tick_font_size=8, max_ticks=4,shade=[True],summary=[True],kde=[False])
#
#    #c.configure(bins=25, rainbow=True)
#    fig = c.plotter.plot(extents=rng)
#    plt.show()
#
#    #amp_ldc[gbmcmc_sources[0]]
#    #amp_ldc[gbmcmc_sources[1]]
#    #amp_ldc[gbmcmc_sources[2]]
#    #ecl_lat_ldc[gbmcmc_sources[0]]
#    #ecl_lat_ldc[gbmcmc_sources[1]]
#    #ecl_lat_ldc[gbmcmc_sources[2]]
#    #ecl_long_ldc[gbmcmc_sources[0]]
#    #ecl_long_ldc[gbmcmc_sources[1]]
#    #ecl_long_ldc[gbmcmc_sources[2]]
#    #dot_freq_ldc[gbmcmc_sources[0]]
#    #dot_freq_ldc[gbmcmc_sources[1]]
#    #dot_freq_ldc[gbmcmc_sources[2]]
#    #inc_ldc[gbmcmc_sources[0]]
#    #inc_ldc[gbmcmc_sources[1]]
#    #inc_ldc[gbmcmc_sources[2]]
#    #phi0_ldc[gbmcmc_sources[0]]
#    #phi0_ldc[gbmcmc_sources[1]]
#    #phi0_ldc[gbmcmc_sources[2]]
#    #pol_ldc[gbmcmc_sources[0]]
#    #pol_ldc[gbmcmc_sources[1]]
#    #pol_ldc[gbmcmc_sources[2]]
#    #
#
#    values1=[]
#    values1.append(freq_ldc[gbmcmc_sources[0]])
#    values1.append(dot_freq_ldc[gbmcmc_sources[0]])
#    values1.append(amp_ldc[gbmcmc_sources[0]])
#    if ecl_long_ldc[gbmcmc_sources[0]] < 0.0:
#        values1.append(ecl_long_ldc[gbmcmc_sources[0]]+2*np.pi)
#    else:
#        values1.append(ecl_long_ldc[gbmcmc_sources[0]])
#    values1.append(np.cos(np.pi/2.0-ecl_lat_ldc[gbmcmc_sources[0]]))
#    values1.append(np.cos(inc_ldc[gbmcmc_sources[0]]))
#    if pol_ldc[gbmcmc_sources[0]] > np.pi:
#        values1.append(pol_ldc[gbmcmc_sources[0]]-np.pi)
#    else:
#        values1.append(pol_ldc[gbmcmc_sources[0]])
#    values1.append(phi0_ldc[gbmcmc_sources[0]])
#
#    values2=[]
#    values2.append(freq_ldc[gbmcmc_sources[1]])
#    values2.append(dot_freq_ldc[gbmcmc_sources[1]])
#    values2.append(amp_ldc[gbmcmc_sources[1]])
#    if ecl_long_ldc[gbmcmc_sources[1]] < 0.0:
#        values2.append(ecl_long_ldc[gbmcmc_sources[1]]+2*np.pi)
#    else:
#        values2.append(ecl_long_ldc[gbmcmc_sources[1]])
#    values2.append(np.cos(np.pi/2.0-ecl_lat_ldc[gbmcmc_sources[1]]))
#    values2.append(np.cos(inc_ldc[gbmcmc_sources[1]]))
#    if pol_ldc[gbmcmc_sources[1]] > np.pi:
#        values2.append(pol_ldc[gbmcmc_sources[1]]-np.pi)
#    else:
#        values2.append(pol_ldc[gbmcmc_sources[1]])
#    values2.append(phi0_ldc[gbmcmc_sources[1]])
#
#    values3=[]
#    values3.append(freq_ldc[gbmcmc_sources[2]])
#    values3.append(dot_freq_ldc[gbmcmc_sources[2]])
#    values3.append(amp_ldc[gbmcmc_sources[2]])
#    if ecl_long_ldc[gbmcmc_sources[2]] < 0.0:
#        values3.append(ecl_long_ldc[gbmcmc_sources[2]]+2*np.pi)
#    else:
#        values3.append(ecl_long_ldc[gbmcmc_sources[2]])
#    values3.append(np.cos(np.pi/2.0-ecl_lat_ldc[gbmcmc_sources[2]]))
#    values3.append(np.cos(inc_ldc[gbmcmc_sources[2]]))
#    if pol_ldc[gbmcmc_sources[2]] > np.pi:
#        values3.append(pol_ldc[gbmcmc_sources[2]]-np.pi)
#    else:
#        values3.append(pol_ldc[gbmcmc_sources[2]])
#    values3.append(phi0_ldc[gbmcmc_sources[2]])
#
#
#    rng=[(min(f0_1)/T,max(f0_1)/T), (min(dfdt_1)/T/T,max(dfdt_1)/T/T), (min(exp(amp_1)),max(exp(amp_1))), (min(phi_1),max(phi_1)), (min(costheta_1),max(costheta_1)), (min(cosi_1),max(cosi_1)), (min(psi_1),max(psi_1)), (min(phi0_1),max(phi0_1))]
#
#    c = ChainConsumer()
#    c.add_chain(datanew1, parameters=[r"$f0 (Hz)$", "df/dt", r"$amp$", r"$\phi$",r"$cos\theta$",r"$cos\iota$", r"$\psi$", r"$\phi_0$"],name="f1").configure(contour_labels="confidence")
#    c.configure(shade_gradient=[0.8],bins=25,colors=["#006b3c"], shade_alpha=0.8, linewidths=[1.0], tick_font_size=8, max_ticks=4,shade=[True],summary=[True],kde=[False])
#    #c.configure(shade_gradient=[0.8],bins=25,colors=["c"], shade_alpha=0.8, linewidths=[1.0], tick_font_size=8, max_ticks=4,shade=[True],summary=[True],kde=[False])
#
#    #c.configure(bins=25, rainbow=True)
#    fig = c.plotter.plot(extents=rng)
...
#
#
#    rng=[(min(f0_2)/T,max(f0_2)/T), (min(dfdt_2)/T/T,max(dfdt_2)/T/T), (min(exp(amp_2)),max(exp(amp_2))), (min(phi_2),max(phi_2)), (min(costheta_2),max(costheta_2)), (min(cosi_2),max(cosi_2)), (min(psi_2),max(psi_2)), (min(phi0_2),max(phi0_2))]
#
#    c = ChainConsumer()
#    c.add_chain(datanew2, parameters=[r"$f0 (Hz)$", "df/dt", r"$amp$", r"$\phi$",r"$cos\theta$",r"$cos\iota$", r"$\psi$", r"$\phi_0$"],name="f2").configure(contour_labels="confidence")
#    c.configure(shade_gradient=[0.8],bins=25,colors=["#9cc6d6"], shade_alpha=0.8, linewidths=[1.0], tick_font_size=8, max_ticks=4,shade=[True],summary=[True],kde=[False])
#    #c.configure(shade_gradient=[0.8],bins=25,colors=["c"], shade_alpha=0.8, linewidths=[1.0], tick_font_size=8, max_ticks=4,shade=[True],summary=[True],kde=[False])
#
#    #c.configure(bins=25, rainbow=True)
#    fig = c.plotter.plot(extents=rng)
...
#
#
#    rng=[(min(f0_3)/T,max(f0_3)/T), (min(dfdt_3)/T/T,max(dfdt_3)/T/T), (min(exp(amp_3)),max(exp(amp_3))), (min(phi_3),max(phi_3)), (min(costheta_3),max(costheta_3)), (min(cosi_3),max(cosi_3)), (min(psi_3),max(psi_3)), (min(phi0_3),max(phi0_3))]
#
#    c = ChainConsumer()
#    c.add_chain(datanew3, parameters=[r"$f0 (Hz)$", "df/dt", r"$amp$", r"$\phi$",r"$cos\theta$",r"$cos\iota$", r"$\psi$", r"$\phi_0$"],name="f3").configure(contour_labels="confidence")
#    c.configure(shade_gradient=[0.8],bins=25,colors=["#7c709c"], shade_alpha=0.8, linewidths=[1.0], tick_font_size=8, max_ticks=4,shade=[True],summary=[True],kde=[False])
#    #c.configure(shade_gradient=[0.8],bins=25,colors=["c"], shade_alpha=0.8, linewidths=[1.0], tick_font_size=8, max_ticks=4,shade=[True],summary=[True],kde=[False])
#
#    #c.configure(bins=25, rainbow=True)
#    fig = c.plotter.plot(extents=rng)
...
#    for i in range(8):
#        ax = axes[i, i]
#        ax.axvline(values[i], color="#006b3c")
#
#
#
#    for yi in range(8):
#        for xi in range(yi):
#            ax = axes[yi, xi]
#            ax.axvline(values[xi], color="#006b3c")
#            ax.axhline(values[yi], color="#006b3c")
#
#
#
#    values=values2
...
#    for i in range(8):
#        ax = axes[i, i]
#        ax.axvline(values[i], color="#9cc6d6")
#
#
#
#    for yi in range(8):
#        for xi in range(yi):
#            ax = axes[yi, xi]
#            ax.axvline(values[xi], color="#9cc6d6")
#            ax.axhline(values[yi], color="#9cc6d6")
#
#
#    values=values3
#    axes = np.array(fig.axes).reshape((8, 8))
#    for i in range(8):
#        ax = axes[i, i]
#        ax.axvline(values[i], color="#7c709c")
#
#
#
#    for yi in range(8):
#        for xi in range(yi):
#            ax = axes[yi, xi]
#            ax.axvline(values[xi], color="#7c709c")
#            ax.axhline(values[yi], color="#7c709c")
#
#    plt.show()
#
#
