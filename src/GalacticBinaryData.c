...
void GalacticBinaryInjectVerificationSource(struct Data ***data_vec, struct Orbit *orbit, struct Flags *flags)
{
  ...
  for(int ii = 0; ii<flags->injection; ii++)
  {
    ...
    gsl_rng_env_setup();
    gsl_rng_set (r, data_vec[ii][0]->iseed);
    ...
    Mc  = chirpmass(m1,m2);
    amp = galactic_binary_Amp(Mc, f0, D, data_vec[0][0]->T);
    
    for(int jj=0; jj<flags->segment; jj++)
    {
      
      struct Data *data  = data_vec[ii][jj];
      ...
      for(int i=0; i<data->N; i++)
      {
        ...
        fprintf(fptr,"%lg %lg %lg %lg %lg",
                f,
                data->tdi->A[2*i],data->tdi->A[2*i+1],
                data->tdi->E[2*i],data->tdi->E[2*i+1]);
        fprintf(fptr,"\n");
      }
      fclose(fptr);
